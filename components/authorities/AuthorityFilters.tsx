...
/**
 * Progressive-enhancement filtering for the authority hub.
 *
...
export function AuthorityFilters({ facets }: { facets: Facet[] }) {
  ...
  useEffect(() => {
    const rows = Array.from(
      document.querySelectorAll<HTMLElement>('[data-authority-row]'),
    );
    let shown = 0;
    ...
    for (const group of Array.from(
      document.querySelectorAll<HTMLElement>('[data-authority-group]'),
    )) {
      const any = Array.from(
        group.querySelectorAll<HTMLElement>('[data-authority-row]'),
      ).some((r) => !r.hidden);
      group.hidden = !any;
    }
    setVisible(shown);
    ...
    <div className="mt-6 rounded-lg border border-ink-100 bg-[#FAFAF7] p-4">
      ...
      <p aria-live="polite" className="mt-3 text-sm text-ink-600">
        {visible === null
          ? null
          : anyActive
            ? `${visible} matching ${visible === 1 ? 'authority' : 'authorities'}`
            : `${visible} authorities`}
      </p>
    </div>
  );
